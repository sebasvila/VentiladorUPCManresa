...
The UPC mechanical ventilation unit follows a periodic profile like
this one shown in Figure~\ref{fig:ventilation-profile}. There is a set
of parameters that are used to control the main characteristics of
this profile. Some of them can be interactively tunned by the end user
while other remain constant. These parameters are shown below.
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[y=0.5cm, x=0.6cm]
    ...
    \draw (end-exp-pau |- 0,-3pt) node[anchor=north] {$T$};
    % y axis annotations
    \draw (-3pt,0) node[anchor=east] {$0$};
    \draw (end-insp-acc-|-3pt,0) node[anchor=east] {$\theta_{r}$};
    \draw (end-insp-lin-|-3pt,0) node[anchor=east] {$\theta_{T}-\theta_r$};
    \draw (end-insp-dec-|-3pt,0) node[anchor=east] {$\theta_{T}$};
    ...
    \draw [thin,loosely dotted] plot (end-exp-pau)  -- (end-exp-pau |-0,-3.7);
  \end{tikzpicture}
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[y=.2cm, x=1cm]
    ...
    \draw (7,-3pt) node[anchor=north] {$\theta_T-\theta_r$};
    \draw (9,-3pt) node[anchor=north] {$\theta_T$};

    \draw (-3pt,0) node[anchor=east] {$0$};
    \draw (-3pt,15) node[anchor=east] {$\omega_r$};
    
    % axis labels      
    \node[below=0.6cm] at (x axis mid) {\small Angle turned (rad)};
    \node[rotate=90, above=0.6cm] at (y axis mid)
    {\small Angular speed ($\mbox{rad}/s$)};
